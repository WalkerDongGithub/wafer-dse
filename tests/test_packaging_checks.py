...
def _sample_cfg() -> dict:
    """与 example_packaging.yaml 一致的配置。"""
    return {
        ...
        "area_per_internal_lane_mm2": 0.025,
        "base_power_w": 20,
        "router_power_w": 0.8,
        "power_per_external_lane_w": 0.35,
        "power_per_internal_lane_w": 0.045,
    }
...
class TestDieAreaCheck(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # lanes_per_target_port = ceil(800/100) = 8
        self.lanes = 8
        self.ports = 16
    ...
    def test_computed_area_matches_manual(self):
        """手工验算面积公式。"""
        result = self.check.run(self.cfg, self.req, self.net, self.lanes, self.ports)
        # router_area = 16 * 1.5 = 24
        # external_area = (16*8) * 0.08 = 10.24
        # internal_area = (144*8) * 0.025 = 28.8
        # die_area = 40 + 24 + 10.24 + 28.8 = 103.04
        self.assertAlmostEqual(result.values["router_area_mm2"], 24.0)
        self.assertAlmostEqual(result.values["external_area_mm2"], 10.24)
        self.assertAlmostEqual(result.values["internal_area_mm2"], 28.8)
        self.assertAlmostEqual(result.values["die_area_mm2"], 103.04)

    def test_different_port_count_changes_area(self):
        """端口数增加 → 面积增加。"""
        r16 = self.check.run(self.cfg, self.req, self.net, self.lanes, 16)
        r32 = self.check.run(self.cfg, self.req, self.net, self.lanes, 32)
    ...
    def test_different_speedup_changes_area(self):
        ...
        r_small = self.check.run(self.cfg, self.req, net_small, self.lanes, 16)
        r_large = self.check.run(self.cfg, self.req, net_large, self.lanes, 16)
        self.assertGreater(
    ...
    def test_fails_when_over_limit(self):
        """将 limit 设为 50 → 面积 103 超标。"""
        cfg = {**self.cfg, "max_die_area_mm2": 50}
        req = _sample_req(max_die_area_mm2=None)  # 让 req 不覆盖
        result = self.check.run(cfg, req, self.net, self.lanes, self.ports)
    ...
    def test_zero_terminals(self):
        ...
        result = self.check.run(self.cfg, self.req, net, self.lanes, 0)
...
class TestPowerCheck(unittest.TestCase):
    ...
    def test_computed_power_matches_manual(self):
        ...
        # internal_power = (144*8) * 0.045 = 51.84
        # total = 20 + 12.8 + 44.8 + 51.84 = 129.44
        self.assertAlmostEqual(result.values["router_power_w"], 12.8)
        self.assertAlmostEqual(result.values["external_power_w"], 44.8)
        self.assertAlmostEqual(result.values["internal_power_w"], 51.84)
        self.assertAlmostEqual(result.values["power_w"], 129.44)

    # —— 通过 / 失败 ——

    def test_passes_when_within_limit(self):
        """power=129 < min(200, 220)=200 → 通过。"""
        result = self.check.run(self.cfg, self.req, self.net, self.lanes, self.ports)
        self.assertTrue(result.passed)

    def test_fails_when_over_req_limit(self):
        """req.max_power_w=50 → power=129 超标。"""
        req = _sample_req(max_power_w=50.0)
        result = self.check.run(self.cfg, req, self.net, self.lanes, self.ports)
        self.assertFalse(result.passed)
        self.assertIn("> limit", result.reason)

    def test_fails_when_over_cfg_limit(self):
        """cfg.max_power_w=50 < req.max_power_w=200 → min=50, power=129 超标。"""
        cfg = {**self.cfg, "max_power_w": 50}
        result = self.check.run(cfg, self.req, self.net, self.lanes, self.ports)
        self.assertFalse(result.passed)

    def test_uses_min_of_req_and_cfg(self):
        """上限取 min(req.max_power_w, cfg.max_power_w)。"""
        # req=200, cfg=100 → limit=100, power=129 → 超标
        cfg = {**self.cfg, "max_power_w": 100}
        result = self.check.run(cfg, self.req, self.net, self.lanes, self.ports)
        self.assertFalse(result.passed,
                         f"power=129 > min(200, 100)=100 should fail")

    # —— 边界 ——

    def test_zero_lanes_zero_power_components(self):
        """lanes=0 → external 和 internal power 为 0。"""
        result = self.check.run(self.cfg, self.req, self.net, 0, 0)
...
class TestExternalIOCheck(unittest.TestCase):
    ...
    def test_budget_calculation(self):
        """external_budget = max_external_lanes / lanes_per_port = 160/8 = 20。"""
        result = self.check.run(self.cfg, self.req, self.net, self.lanes, self.ports)
    ...
    def test_passes_when_ports_fit_budget(self):
        """16 ports ≤ 20 → 通过。"""
        result = self.check.run(self.cfg, self.req, self.net, self.lanes, 16)
        self.assertTrue(result.passed)

    def test_fails_when_ports_exceed_budget(self):
        """30 ports > 20 budget → 失败。"""
        result = self.check.run(self.cfg, self.req, self.net, self.lanes, 30)
        self.assertFalse(result.passed)
        self.assertIn("external budget", result.reason)

    def test_exact_boundary(self):
        """恰好等于 budget → 通过。"""
        result = self.check.run(self.cfg, self.req, self.net, self.lanes, 20)
        self.assertTrue(result.passed)

    def test_budget_changes_with_lane_rate(self):
        """lane_rate 越高 → 每端口 lane 越少 → budget 越多。"""
        # lanes=4 (ceil(800/200)=4), budget=160/4=40
        result = self.check.run(self.cfg, self.req, self.net, lanes_per_target_port=4, port_count=16)
...
class TestInternalIOCheck(unittest.TestCase):
    ...
    def test_budget_calculation(self):
        """internal_budget = max_internal_lanes / lanes_per_port = 1600/8 = 200。"""
        result = self.check.run(self.cfg, self.req, self.net, self.lanes, self.ports)
        self.assertAlmostEqual(result.values["internal_budget_links"], 200.0)
        self.assertAlmostEqual(result.values["required_internal_lanes"], 1152.0)
    ...
    def test_fails_when_links_exceed_budget(self):
        ...
        result = self.check.run(self.cfg, self.req, net, self.lanes, self.ports)
        self.assertFalse(result.passed)
        self.assertIn("internal_links", result.reason)

    def test_exact_boundary(self):
        """恰好等于 budget → 通过。"""
        net = _sample_net(required_internal_800g_links=200)
        result = self.check.run(self.cfg, self.req, net, self.lanes, self.ports)
        self.assertTrue(result.passed)
...
class TestPackagingModelOrchestrator(unittest.TestCase):
    ...
    def test_mesh4_det_all_pass(self):
        """标准 16 端口 mesh4x4 应全部通过。"""
        est = self.model.estimate(self.req, self.net)
        self.assertTrue(est.area_ok)
        self.assertTrue(est.power_ok)
        self.assertTrue(est.external_ports_ok)
        self.assertTrue(est.internal_links_ok)
    ...
    def test_details_contains_all_check_values(self):
        est = self.model.estimate(self.req, self.net)
        self.assertIn("lanes_per_target_port", est.details)
    ...
    def test_lanes_per_port_ceiling(self):
        """800G / 100G = 8 lanes；801G → 9 lanes。"""
        req = _sample_req(
            target_nonblocking_gbps_per_port=801.0,
            packaging_config="configs/example_packaging.yaml",
        )
        est = self.model.estimate(req, self.net)
        self.assertAlmostEqual(est.details["lanes_per_target_port"], 9.0)
