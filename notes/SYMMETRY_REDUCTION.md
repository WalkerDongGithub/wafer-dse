...
**假设 1（单位收发量 / 双随机）** 每个节点恰好发出总计 1 单位流量、也恰好接收总计 1 单位流量，即 $\sum_j t_{ij} = 1$ 且 $\sum_i t_{ij} = 1$，$\forall i,j$。物理上对应"每个端口以满速率同时收发"。这一双随机约束使 $\mathcal{T}$ 成为 Birkhoff 多面体。
...
其中 $\mathcal{T} = \{T \in [0,1]^{n \times n} : \sum_j t_{ij} = 1,\; \sum_i t_{ij} = 1,\; \forall i,j\}$（Birkhoff 多面体，双随机矩阵集）。
