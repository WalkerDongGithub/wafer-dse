...
> 结果预览数字全部来自 DataSteward 报告（可复现）：E1 排序稳健（Spearman ρ=1.000, n=11）、E3B v2 耦合分歧（10/72 构型 rel_diff>0.01，max 0.80，Mesh(3) sep=1075 vs joint=5363）。
...
5. **E3B v2 处理**：10/72 分歧即 E3B v2 的结论；摘要不写实验代号，实验代号只在 §5.3 出现。
