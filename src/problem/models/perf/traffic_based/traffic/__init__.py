...
class Selector(abc.ABC):
    ...
    不同策略实现同一接口, 彼此可互换:
      - ConjugacySelector   S_n 共轭类 (保守近似)
      - DerangementSelector 暴力枚举全部 derangement (n ≤ 8)
      - ManualSelector      手工指定排列
      - TODO                Aut(G) 轨道计算 (精确归约)
    """
...
class PermutationPattern(Pattern):
    ...
    sigma[i] = i 发往的目标终端索引.
    和 TrafficMatrixPattern 不同: 冻结 (hashable), 带 sigma 访问.
    """
...
def select_representatives(
    ...
    Args:
      ...
      selector: 自定义选择器. 未指定时用 ConjugacySelector.

    返回类型是 list[Pattern], 调用方不感知具体 Pattern 子类.
    """
    if selector is None:
        from problem.models.perf.traffic_based.traffic._conjugacy import ConjugacySelector
...
# 重新导出（在文件末尾以避开循环导入）
from problem.models.perf.traffic_based.traffic._conjugacy import ConjugacySelector  # noqa: E402
from problem.models.perf.traffic_based.traffic._brute import DerangementSelector    # noqa: E402
from problem.models.perf.traffic_based.traffic._manual import ManualSelector         # noqa: E402
