...
> **代码服务于 V5 数理模型**（`notes/MATH_MODEL_V5_JOINT_SENSITIVITY.md`，全文唯一权威模型文档）：
> - 代码与 V5 冲突 → 先改文档达成共识，再改代码
> - V5 没提到的（产品级校验、异常处理、边界检查）→ 可以删
> - V5 需要的（运行结果、日志、实验数据）→ 必须做好
> - `docs/paper/`（论文 LaTeX）是 V5 的**下游产物**，从 V5 派生，不反向约束模型
> - **这不是一个产品，这是一套数理模型的配套代码。**
>
> **上位意图**：`insight.md`（7 条 critical insight，字节级不变）是全项目核心意图；V5 是其形式化，代码是 V5 的实现——三者一以贯之。
...
**类 docstring**：中文，一句话——这个类做什么、对应 V5 哪条约束（如果是 Model 子类）。
